...
const Clasificacion = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserLeague = async () => {
      try {
        ...
        const checkResponse = await fetch("http://localhost:8086/api/leagues/user/check", {
          headers: {
        ...
        if (checkData.hasLeague) {
          const leagueResponse = await fetch(`http://localhost:8086/api/leagues/${checkData.leagueId}`, {
            headers: {
              "Authorization": `Bearer ${token}`,
              "Accept": "application/json"
            }
  ...
  const fetchClasificacion = async (leagueId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`http://localhost:8086/api/leagues/${leagueId}/ranking`, {
        headers: {
          "Authorization": `Bearer ${token}`,
          "Accept": "application/json"
        }
      ...
      if (response.ok) {
        ...
        const miembrosResponse = await fetch(`http://localhost:8086/api/leagues/${leagueId}/users`, {
          headers: {
            "Authorization": `Bearer ${token}`,
            "Accept": "application/json"
          }
  ...
  const fetchMiembrosLiga = async (leagueId) => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`http://localhost:8086/api/leagues/${leagueId}/users`, {
        headers: {
          "Authorization": `Bearer ${token}`,
          "Accept": "application/json"
        }
